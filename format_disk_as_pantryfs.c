...
int main(int argc, char *argv[])
{
	...
	char *names_contents = "You Zhou, Aoxue Wei, and Panyu Gao.\n";
	...
	SETBIT(sb.free_inodes, 2);	// inode for members/
	SETBIT(sb.free_inodes, 3);	// inode for names.txt

	SETBIT(sb.free_data_blocks, 0);
	SETBIT(sb.free_data_blocks, 1);
	SETBIT(sb.free_data_blocks, 2);	// data block for memebers/
	SETBIT(sb.free_data_blocks, 3); // data block for names.txt
	...
	ret = write(fd, zeroes, len);
	passert(ret == len, "Pad to end of hello.txt data block");
